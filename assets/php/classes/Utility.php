...
class Utility {
    static function swc_time($time, $offset = FALSE){ // TRUE to convert real time() to swc_time. FALSE to convert swc timestamps to swc_time.
    ...
    static function swc_timestamp($year, $day, $hour = 0, $minute = 0, $second = 0) {
        $real_timestamp = Utility::real_time($year, $day, $hour, $minute, $second);
        $swc_timestamp = Utility::swc_time($real_timestamp);
        return $swc_timestamp['timestamp'];
    }
    
    static function real_time($year, $day, $hour = 0, $minute = 0, $second = 0){
        $years = $year * 31536000;
        $days = $day * 86400;
        $hours = $hour * 3600;
        $minutes = $minute * 60;
    ...
    static function add_character($handle){
        global $db;
    ...
    static function verify_character($handle){
        $handle = urlencode(str_replace(" ","%20",$handle));
        $xml = simplexml_load_string(file_get_contents("https://www.swcombine.com/ws/v2.0/character/handlecheck/".$handle));
        $uid = mysqli_real_escape_string($db->connection, $xml->uid);
        $handle = mysqli_real_escape_string($db->connection, $xml->handle);
    ...
    static function add_character_affiliation($character_uid,$faction,$time,$source){
        global $db;
    ...
    static function checkFilters($user_id){
        $db = new database;
        $db->connect();
    ...
    static function compileFilters($filters){
        $db = new database;
        $db->connect();
